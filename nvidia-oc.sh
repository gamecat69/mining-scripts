...
output "$GREEN" "[i] Found $NUMGPU Nvidia cards"
...
if [ "$LIMITPOWER" = "yes" ] ; then
   ...
   do
      ...
      output "$BLUE" "[i] Limiting  GPU:$n power to ${POWERLIMIT_WATTS[$n]} Watts. Minimum for this card: $MINWATT Watts"
      nvidia-smi -i $n -pm 1
...
if [ "$GPUOC" = "yes" ] ; then
   ...
   while [ $n -lt $NUMGPU ];
   do
      output "$BLUE" "[i] Overclocking GPU:$n by ${GPUOVERCLOCK[$n]}"
...
if [ "$MEMOC" = "yes" ] ; then
   ...
   while [ $n -lt $NUMGPU ];
   do
      output "$BLUE" "[i] Overclocking GPU:$n memory by ${MEMOVERCLOCK[$n]}"
...
if [ "$MAXPERF" = "yes" ] ; then
   ...
   output "$BLUE" "[i] Setting Powermizer to Prefer Max Performance"
   n=0
   while [  $n -lt  $NUMGPU ];
   do
